...
<template>
	<form @submit.prevent="onSubmit">
		<label for="mail-gmail-oauth-tenant-id"> {{ t('mail', 'Tenant ID') }} </label>
		<input
			id="mail-gmail-oauth-tenant-id"
			v-model="tenantIdVal"
			:disabled="loading"
			type="text"
			required>
...
export default {
	...
	methods: {
		async onSubmit() {
			this.loading = true
			try {
				await configure(this.tenantId, this.clientIdVal, this.clientSecret)
